...
class formHelper {
    ...
    protected function _getLabel(array $args, $formElement) {
        ...
        if (isset($args['label'])) {
            $label = (!isset($args['htmlok']) || !$args['htmlok'] ? get::htmlentities($args['label']) : $args['label']);
    ...
    public function input(array $args) {
        ...
        $properties = $this->_getProperties($args, array('type', 'maxlength', 'placeholder'));
    ...
    /**
     * Cache containing individual radio, checkbox and dateSelect-selector elements in an array
     * @var array
     */
     ...
     * 'id' => '',
     * 'legend' => '',
     * 'options' => array('value1' => 'text1', 'value2' => 'text2', 'value3' => 'text3'), // if just one option exists this can be a string
    ...
    public function radios(array $args) {
        ...
        if (!empty($args['options']) && !is_array($args['options'])) {
            $args['options'] = (isset($args['label']) ? array($args['options'] => $args['label']) : array($args['options']));
    ...
    public function checkbox(array $args) {
        if (empty($args['options']) && !empty($args['label'])) {
            $args['options'] = $args['label'];
        }
        if (is_array($args['options'])) {
            $args['options'][$args['name']] = $args['options'];
        }
        return $this->checkboxes($args);
    }
     ...
     * @var array
     */
    public $months = array(1 => '01 - January', '02 - February', '03 - March', '04 - April',
    ...
    public function dateSelect(array $args) {
        $includeTime = ((isset($args['time']) && $args['time'])
                        || isset($args['hour']) || isset($args['minute']) || isset($args['mil']));
        ...
        if ($includeTime) {
            ...
            if (!isset($args['ampm']) && isset($args['pmam'])) {
                $args['ampm'] = $args['pmam'];
            }
            if (!$isMil) {
                $dateParts['ampm'] = 'p';
            }
            if (!isset($args['hour']['options'])) {
                $args['hour']['options'] = (!$isMil ? range(1, 12) : range(0, 23));
            }
    ...
    public function hiddens(array $args = array()) {
        foreach ($args as $key => $val) {
            $return[] = $this->hidden($key, $val);
        }
        return (isset($return) ? implode($return) : '');
    }
    ...
    public function __call($name, array $args) {
        $inputShorthand = array('text', 'textarea', 'email', 'password', 'file', 'hidden', 'submit', 'button', 'image');
        if (in_array($name, $inputShorthand)) {
            if (isset($args[0]) && !is_array($args[0]) && $args[0]) { //shortcut strings
                $defaultKey = 'name';
                if ($name == 'submit' && !isset($args[1])) {
                    $defaultKey = 'value';
                }
