...
class toolsHelper {
    ...
    public function youtubeImageUrl($id, $quality = 'hq') {
        $qualityOptions = array(
            'hq' => 'hqdefault',
            'mq' => 'mqdefault',
            'sd' => 'sddefault', //not on all videos
            'max' => 'maxresdefault', //not on all videos
            'full' => 'default',
            '0' => '0',
            '1' => '1',
            '2' => '2',
        ...
        $quality = (isset($qualityOptions[$quality]) ? $qualityOptions[$quality] : current($qualityOptions));
     ...
     *
     * @param mixed $args can be a string with Youtube ID or URL, or an array of args that contains at least a 'v' key
     * @param mixed $args2 Optional, used only when $args is a string
     * @return string
     */
    public function youtube($args, $args2 = array()) {
        if (!is_array($args)) {
            $id = $args;
            $args = $args2;
            $args['v'] = $id;
        ...
        $youtubeUrls = array('http://www.youtube.com/watch?v=', 'https://www.youtube.com/watch?v=',
                             'http://www.youtube.com/v/', 'https://www.youtube.com/v/');
        $id = str_replace($youtubeUrls, '', $args['v']);
        if (($queryStart = strpos($id, '&')) !== false) {
            $queryString = str_replace('&', '&amp;', substr($id, $queryStart));
            $id = substr($id, 0, $queryStart);
        }
        $url = '//www.youtube.com/embed/' . $id . '?hl=en&amp;fs=1&amp;wmode=opaque';
        ...
        if (isset($args['autoplay'])) {
            $url .= '&amp;autoplay=' . (int) (bool) $args['autoplay'];
        }
        if (!isset($args['html5'])) {
            $args['html5'] = '1';
        }
        ...
        if (!isset($args['allowFullScreen'])) {
            $args['allowFullScreen'] = '1';
        }
        $url .= '&amp;allowFullScreen=' . $args['allowFullScreen'];
        $url .= '&amp;modestbranding=' . $args['modestbranding'];
        $url .= '&amp;rel=' . $args['rel'];
        if (!empty($args['cc'])) { //can be boolean or a 2-letter country-code
            $url .= '&amp;cc_load_policy=1&amp;cc_lang_pref=' . (strlen($args['cc']) != 2 ? 'en' : $args['cc']);
        }

        return '<iframe class="youtube-player" type="text/html" frameborder="0" allowfullscreen="'
             . $args['allowFullScreen'] . '" '
             . htmlHelper::universalProperties($args)
             . ' width="' . $args['width'] . '" height="' . $args['height']  . '" src="' . $url . '"></iframe>';
    }
...
var codeAddress = function(map, address, thisMarkerArgs) {
    ...
    geocoder.geocode({"address": address}, function(results, status) {
        ...
        $js .= 'var googlemapDiv' . $this->_mapCount . ' = document.getElementById("googlemap' . $this->_mapCount . '");
...
var googlemap' . $this->_mapCount . ' = function() {
    ...
    codeAddress(googlemapObj' . $this->_mapCount . ', "' . $location . '"';
        ...
        $eof .= $html->jsInline($js);
        $html->eof[] = $eof;
        return '<div id="googlemap' . $this->_mapCount . '"></div>';
    }
    ...
    protected $_meetupPhotoSizes = array('thumbnail' => 'thumb', 'small' => 'global', 'medium' => 'event', 'large' => '600', 'original' => 'highres');

    public function meetupPhotoUrl($photoUrl, $size = 'small') {
        $size = strtolower($size);
        if (!isset($this->_meetupPhotoSizes[$size])) {
            $size = 'small';
        }
        $bits = explode('/', $photoUrl);
        $lastBit = array_pop($bits);
        $lastBit = strstr($lastBit, '_');
        $bits[] = $this->_meetupPhotoSizes[$size] . $lastBit;
        return implode('/', $bits);
    }
    ...
    /**
     * returns current age from birthdate
     *
     * @param int $yearOrTs either timestamp of birthdate or birth-year
     * @param int $month birth-month (with timestamp, leave this null)
     * @param int $day birth-day (with timestamp, leave this null)
     * @return int
     */
    public function age($yearOrTs, $month = null, $day = null) {
        if (empty($month)) { //TS supplied
            $day = date('d', $yearOrTs);
            $month = date('m', $yearOrTs);
            $year = date('Y', $yearOrTs);
        } else {
            $year = $yearOrTs;
        }
        $ts = mktime(0, 0, 0, $month, $day, $year);
        $age = date('Y') - $year;
        $birthdate = date('md', date('U', $ts));
        if ($birthdate > date('md')) {
            --$age;
        }
        return $age;

        //date_diff is added in PHP 5.3
        //return date_diff(date_create(empty($day) ? date('c', $yearOrTs) : $yearOrTs . '/' . $month . '/' . $day), date_create('today'))->y;
    }
